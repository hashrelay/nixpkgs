...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.patroni-setup = {
      after = [
        "network.target"
        "patroni.service"
      ];
      ...
      script =
        ''
          set -xeu -o pipefail
              ...
              PSQL="patronicli query -U postgres"
        ...
        + lib.optionalString (cfg.ensureDatabases != [ ]) ''
          ${lib.concatMapStrings (database: ''
            $PSQL -c "SELECT 1 FROM pg_database WHERE datname = '${database}'" | grep -q 1 || $PSQL -c 'CREATE DATABASE "${database}"'
          '') cfg.ensureDatabases}
        ''
        + ''
          ${lib.concatMapStrings (
            user:
            let
              dbOwnershipStmt = lib.optionalString user.ensureDBOwnership ''$PSQL -c 'ALTER DATABASE "${user.name}" OWNER TO "${user.name}";' '';

              filteredClauses = lib.filterAttrs (name: value: value != null) user.ensureClauses;

              clauseSqlStatements = lib.attrValues (
                lib.mapAttrs (n: v: if v then n else "no${n}") filteredClauses
              );

              userClauses = ''$PSQL -c 'ALTER ROLE "${user.name}" ${lib.concatStringsSep " " clauseSqlStatements}' '';
            in
            ''
              $PSQL -c "SELECT 1 FROM pg_roles WHERE rolname='${user.name}'" | grep -q 1 || $PSQL -c 'CREATE USER "${user.name}"'
              ${userClauses}
